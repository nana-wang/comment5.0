...
class OAuthRequest {
    ...
    public function to_header ($realm = null) {
        ...
        foreach ($this->parameters as $k => $v) {
            ...
            if (is_array($v)) {throw new OAuthException(
...
class OAuthServer {
 ...
 // hi blaine
    ...
    private function get_version (&$request) {
        ...
        if ($version !== $this->version) {throw new OAuthException(
    ...
    private function get_signature_method (&$request) {
        ...
        if (! $signature_method) {            // According to chapter 7 ("Accessing Protected Ressources") the
            // signature-method
            // parameter is required, and we can't just fallback to PLAINTEXT
            throw new OAuthException(
                    'No signature method parameter. This parameter is required');}
        
        if (! in_array($signature_method, array_keys($this->signature_methods))) {throw new OAuthException(
    ...
    private function check_timestamp ($timestamp) {
        if (! $timestamp) throw new OAuthException(
                'Missing timestamp parameter. The parameter is required');
        
        // verify that timestamp is recentish
        $now = time();
        if (abs($now - $timestamp) > $this->timestamp_threshold) {throw new OAuthException(
    ...
    private function check_nonce ($consumer, $token, $nonce, $timestamp) {
        if (! $nonce) throw new OAuthException(
                'Missing nonce parameter. The parameter is required');
        
        // verify that the nonce is uniqueish
        $found = $this->data_store->lookup_nonce($consumer, $token, $nonce, 
                $timestamp);
...
class TwitterOAuth {
    ...
    function getXAuthToken ($username, $password) {
        ...
        $request = $this->oAuthRequest($this->accessTokenURL(), 'POST', 
                $parameters);
        $token = OAuthUtil::parse_parameters($request);
        $this->token = new OAuthConsumer($token['oauth_token'], 
                $token['oauth_token_secret']);
    ...
    function get ($url, $parameters = array()) {
        ...
        if ($this->format === 'json' && $this->decode_json) {return json_decode(
                    $response);}
        return $response;
    }
