...
function add(){
 		...
 		if( fourm_item_account == ''){
 			alert(chosegroup);return false;
 		}
 		var fourm_item_title =$("input[name='fourm_item_title']").val();
		if( fourm_item_title == ''){
			alert(columnnamenoempty);return false;
		}
 		var fourm_item_idtype =$(":radio[name='fourm_item_idtype']:checked").val();
		if( fourm_item_idtype == null ){
			alert(pleaseselecttypegenus);return false;
		}
	    ...
	    //
 		...
 		if( fourm_item_idtype == 3 ){
 			 ...
 			 for(var n=0;n < tag.length;n++){
				var a = tag[n].value;
				var reg = /^[0-9a-zA-Z\u4e00-\u9fa5_]+#[0-9]+#[0-9]+$/;
				if(reg.test(a) == false){
					alert(fillinthetag);return false;
				}
				var b = document.getElementsByName("tags[]")[n].getAttribute("item_ext_id");
				fourm_item_tag.push(a+'#'+b);
		    }
			//console.log(fourm_item_tag);return false;
			...
			if( fourm_item_tag_type == null ){
				alert(pleaseselectlabeltype);return false;
			}else{
				if(isNaN(tag_content_online)){
					alert(tagupperlimiterror);return false;
				}else{
			...
			if( img_content_online == '' ){
				alert(uploadtagupperlimitnoempty);return false;
			}else{
				if(isNaN(img_content_online)){
					alert(uploadtagupperlimitisnumber);return false;
				}
			}
			if( img_content_path == '' ){
				alert(serverpathcannotnoempty);return false;
			}
			if( img_content_type == null ){
				alert(pleaseselectpicturetype);return false;
			}
 			 var fourm_item_tag = []; // 标签类型存放为空，图片类型时存放图片中表情的id
			...
			if( word_content_prompt == ''){
				alert(prompttextcannotnoempty);return false;
			}
			if( word_content_online == '' ){
				alert(textlinenoempty);return false;
			}else{
				if(isNaN(word_content_online)){
					alert(textlinecanonlynumbers);return false;
				}else{
					var param = {id:id,fourm_item_account:fourm_item_account,fourm_item_title:fourm_item_title,fourm_item_idtype:fourm_item_idtype,
						fourm_item_is_ver:fourm_item_is_ver,item_ext_delid:item_ext_delid,
...
function formtype_add(){
		...
		if( fourm_title == '' ){
			alert(formtypenamenoempty);return false;
		}
		if( fourm_account == ''){
			alert(chosegroup);return false;
		}
	    //空
		if( fourm_idtype_id == '' ){
			alert(formtypenoempty);return false;
		}
		if( fourm_order == '' ){
			alert(pleaseselectthesort);return false;
		}
		if( fourm_meth == '' ){
			alert(pleaseselectthemethod);return false;
		}
		if( fourm_pess == '' ){
			alert(pleaseselectalicense);return false;
		}
		if( fourm_number == '' ){
			alert(pleaseselectcomments);return false;
		}
		if( fourm_reply == '' ){
			alert(pleaseselectisreply);return false;
		}
		if( fourm_anonymous == '' ){
			alert(pleaseselectisanonymous);return false;
		}
...
<!-- 		    } -->
<!-- 		}  -->
	     $("#formtype_button_add").text(editdo);
		$('.box-default-edit').removeClass("collapsed-box");
		$('.box-body-edit').css('display','block');
...
<!-- // 表单类型清除表单类型  清除-->
function category_clear(action_remove_id,action_add_id){
	    ...
	    $("input[name='category_id']").val('');
 		...
 		$("#formtype_button_add").text(newadd);
		//清除select
		var inform = document.getElementById(action_remove_id);
		for(var i=0;i < inform.length;i++){
			$('<option>').val(inform.options[i].value).text(inform.options[i].text).appendTo('#'+action_add_id);
		}
		$('#'+action_remove_id).empty();
}
...
function usearea_view_success(data){
	...
	if(data.flg == true ){
		...
		$("#usearea_button_add").text(editdo);
	}else{
		$('#userarea_error').show();
	}
}
...
function tag_del(e){
	...
	var tag = document.getElementsByName("tags[]").length;
	if( tag < 5){
		$('#add_tags').show();
	}
}
function tag_html(val){
		var html ='<div class="form-group tags_input" >';
 		if( val == ''){
		   html +='<label class="col-sm-2 control-label" for="firstname"><small class="label pull-bottom bg-red"><span class="item_delete" item_ext_id="" onclick="tag_del(this);" style="cursor:pointer;">'+deletes+'</span></small></label>';
		}else{
			html +='<label class="col-sm-2 control-label" for="firstname"><small class="label pull-bottom bg-red"><span class="item_delete" item_ext_id="'+ +val['id']+'" onclick="tag_del(this);" style="cursor:pointer;">'+deletes+'</span></small></label>';
		}
		html +='<div class="col-sm-5">';
		if( val == ''){
		  html +='<input type="text" placeholder="'+tagsort+'" value="'+val+'"  item_ext_id=""  name="tags[]" class="form-control">&nbsp;'; 
		}else{
			html +='<input type="text" placeholder="'+tagsort+'" value="'+val['item_tag_name']+'#'+val['item_tag_score']+'#'+val['item_tag_sort']+'" item_ext_id="'+ +val['id']+'" name="tags[]" class="form-control">&nbsp;'; 
		}
		html +='</div>';
		html +='</div>'; 
		return html;
...
$("#add_tags").click(function(event){
	var html = tag_html('');
	var tag_type =$(":radio[name='fourm_item_tags_type']:checked").val();
...
function makeing_form(){
	    ...
	    if( fourm_generate_area == '' ){
			alert(chosegroup);return false;
		}
		if(fourm_category_id == '' ){
			alert(choseformtype);return false;
		}
		$.ajax({
            type: "POST",
...
function idtype_setting(id){
	...
	if(id==1 || id==2){
		...
		$('#tag_content_online').val(5);
		 ...
		 for(var n=0;n < tag.length;n++){
			 if(tag.length>5){
				 var thisNode=document.getElementsByName("tags[]")[5].parentNode.parentNode;
