...
// ��ȡtoken1
   ...
   *
   *Ч�ú���4��������URL����ĸ�ʽ������
   *����ṩ����ȫ�޶���URI,����κ�multi-slashes(Э�����)�����滻Ϊһ��һ�����
  ...
  private function curlit() {
    ...
    switch ($this->request_settings['method']) {
      case 'GET':
        ...
        break;
      case 'POST':
        curl_setopt($c, CURLOPT_POST, true);//// 发送一个常规的Post请求
        if (isset($this->request_settings['postfields']))
          $postfields = $this->request_settings['postfields'];
        else
          $postfields = array();

        curl_setopt($c, CURLOPT_POSTFIELDS, $postfields);// Post提交的数据包
        break;
      default:
        if (isset($this->request_settings['postfields']))
    ...
    curl_setopt_array($c, array(
      ...
      CURLOPT_USERAGENT      => $this->config['user_agent'], // 模拟用户使用的浏览器
      CURLOPT_CONNECTTIMEOUT => $this->config['curl_connecttimeout'],
      CURLOPT_TIMEOUT        => $this->config['curl_timeout'],// 设置超时限制防止死循环
      CURLOPT_RETURNTRANSFER => true,// 获取的信息以文件流的形式返回
      CURLOPT_SSL_VERIFYPEER => $this->config['curl_ssl_verifypeer'],// 对认证证书来源的检查
      CURLOPT_SSL_VERIFYHOST => $this->config['curl_ssl_verifyhost'], // 从证书中检查SSL加密算法是否存在
      CURLOPT_FOLLOWLOCATION => $this->config['curl_followlocation'],// 使用自动跳转
      CURLOPT_PROXY          => $this->config['curl_proxy'],
      CURLOPT_ENCODING       => $this->config['curl_encoding'],
      CURLOPT_URL            => $this->request_settings['url'],// 显示返回的Header区域内容
      // process the headers
      CURLOPT_HEADERFUNCTION => array($this, 'curlHeader'),
      CURLOPT_HEADER         => false,// 显示返回的Header区域内容
