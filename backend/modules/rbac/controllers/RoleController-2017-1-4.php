...
class RoleController extends Controller {
    ...
    public function actionDelete ($id) {
    	...
    	try {
            ...
            // 规则角色表删除auth_rule
            $_auth_model = Rule::findOne(
                    [
                            'name' => $id
                    ]);
            if ($_auth_model->delete()) {
                // 如果删除角色授权限信息删除
                // 规则角色表删除auth_rule
                $res_auth_model = Userassign::findOne(
                        [
                                'role_name' => $id
                        ]);
                
                $res_auth_item = AuthItem::findOne(
                		[
                		'name' => $id
                		]);
                try {
                    if ($res_auth_model->delete() && $res_auth_item->delete() ) {} else {
                ...
                throw new NotFoundHttpException('角色关系删除失败!');
            }
        } catch (\Exception $e) {
        	$transaction->rollback();
        	throw new NotFoundHttpException('操作失败!' . __LINE__);
        }
    }
