 ...
 * ~~~
 *
 * @property string $mainLayout Main layout using for module. Default to layout
 *           of parent module.
 *           Its used when `layout` set to 'left-menu', 'right-menu' or
 *           'top-menu'.
...
class Module extends \yii\base\Module {
    ...
    /**
     *
     * @var string Main layout using for module. Default to layout of parent
     *      module.
     *      Its used when `layout` set to 'left-menu', 'right-menu' or
     *      'top-menu'.
     */
    ...
    public function init () {
        ...
        if ($this->navbar === null) {
            $this->navbar = [
                    [
                            'label' => Yii::t('rbac-admin', 'Help'),
                            'url' => 'https://github.com/mdmsoft/yii2-admin/blob/master/docs/guide/basic-usage.md'
                    ],
                    [
    ...
    public function getMenus () {
        if ($this->_normalizeMenus === null) {
            ...
            foreach ($this->_coreItems as $id => $lable) {
                if ($id !== 'menu' ||
                         ($config->db !== null &&
                         $config->db->schema->getTableSchema($config->menuTable) !==
                         null)) {
                    $this->_normalizeMenus[$id] = [
                            'label' => Yii::t('rbac-admin', $lable),
                            'url' => [
                                    $mid . $id
                            ]
                    ];
            ...
            foreach ($this->_menus as $id => $value) {
                if (empty($value)) {
                    ...
                    $this->_normalizeMenus[$id] = isset(
                            $this->_normalizeMenus[$id]) ? array_merge(
                            $this->_normalizeMenus[$id], $value) : $value;
                    if (! isset($this->_normalizeMenus[$id]['url'])) {
                        $this->_normalizeMenus[$id]['url'] = [
                                $mid . $id
                        ];
                    }
                }
