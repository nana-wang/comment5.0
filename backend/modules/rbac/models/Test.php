...
class CategoryModel extends \yii\easyii\components\ActiveRecord {
    ...
    public function rules () {
        return [
                ...
                [
                        ...
                        'message' => Yii::t('easyii', 
                                'Slug can contain only 0-9, a-z and "-" characters (max: 128).')
                ],
                [
                        'slug',
                        'default',
    ...
    public function behaviors () {
        return [
                'cacheflush' => [
                        ...
                        'key' => [
                                static::tableName() . '_tree',
                                static::tableName() . '_flat'
                        ]
                ],
    ...
    public function beforeSave ($insert) {
        if (parent::beforeSave($insert)) {
            if (! $insert && $this->image != $this->oldAttributes['image'] &&
                     $this->oldAttributes['image']) {
                @unlink(
                        Yii::getAlias('@webroot') . $this->oldAttributes['image']);
            }
            return true;
        } else {
            return false;
    ...
    public static function generateTree () {
        $collection = static::find()->with('seo')
            ->sort()
            ->asArray()
            ->all();
        $trees = array();
