...
class MenuHelper {
     ...
     * @return array
     */
    public static function getAssignedMenu ($userId, $userData, $root = null, 
        ...
        /**
         * $redis = Yii::$app->redis;
         * if ($redis->exists(md5('MenuTreeData'))) {
         * $menus = unserialize($redis->get(md5('MenuTreeData')));
         * } else {
         * $menus = Menu::find()->asArray()
         * ->indexBy('id')
         * ->all();
         * $redis->set(md5('MenuTreeData'), serialize($menus));
         * }
         */
        $menus = Menu::find()->asArray()
            ->indexBy('id')
        ...
        $key = [
                __METHOD__,
                $userId,
                $manager->defaultRoles
        ];
        $cache = $config->cache;
        if ($refresh || $cache === null ||
                 ($assigned = $cache->get($key)) === false) {
            ...
            $assigned = [];
            $query = Menu::find()->select(
                    [
                            'id'
                    ])->asArray();
            if (count($filter2)) {
                $assigned = $query->where(
                        [
                                'route' => $filter2
                        ])->column();
            }
            ...
            // $manager = Yii::$app->getAuthManager();
            // $data = $manager->getPermissionsByUser($userId);
            // var_dump($data);exit;
            $assigned = static::requiredParent($assigned, $menus);
            if ($cache !== null) {
                $cache->set($key, $assigned, $config->cacheDuration, 
                        new TagDependency(
                                [
                                        'tags' => self::CACHE_TAG
                                ]));
        ...
        if ($refresh || $callback !== null || $cache === null ||
                 (($result = $cache->get($key)) === false)) {
            $result = static::normalizeMenu($assigned, $menus, $callback, $root);
            if ($cache !== null && $callback === null) {
                $cache->set($key, $result, $config->cacheDuration, 
                        new TagDependency(
                                [
                                        'tags' => self::CACHE_TAG
                                ]));
     ...
     * @return array
     */
    private static function normalizeMenu (&$assigned, &$menus, $callback, 
            $parent = null) {
        $result = [];
        $order = [];
        foreach ($assigned as $id) {
            ...
            if ($menu['parent'] == $parent) {
                $menu['children'] = static::normalizeMenu($assigned, $menus, 
                        $callback, $id);
                if ($callback !== null) {
    ...
    public static function invalidate () {
        if (Configs::instance()->cache !== null) {
            TagDependency::invalidate(Configs::instance()->cache, 
                    self::CACHE_TAG);
        }
    }
}
