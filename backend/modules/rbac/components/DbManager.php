...
/**
 * DbManager represents an authorization manager that stores authorization
 ...
 * data by setting [[\yii\rbac\DbManager::$itemTable]],
 * [[\yii\rbac\DbManager::$itemChildTable]] and
 * [[\yii\rbac\DbManager::$assignmentTable]].
 *
...
class DbManager extends \yii\rbac\DbManager {
    ...
    /**
     *
     * @var array itemName => childName[]
     */
    private $_children;

    /**
     *
     * @var array userId => itemName[]
     */
    ...
    public function checkAccess ($userId, $permissionName, $params = []) {
        ...
        return $this->checkAccessRecursive($userId, $permissionName, $params, 
                $assignments);
    }
     ...
     *
     */
    protected function checkAccessRecursive ($user, $itemName, $params, 
            $assignments) {
        ...
        if (isset($assignments[$itemName]) ||
                 in_array($itemName, $this->defaultRoles)) {return true;}
        
        foreach ($this->_children as $parentName => $children) {
            if (in_array($itemName, $children) &&
                     $this->checkAccessRecursive($user, $parentName, $params, 
    ...
    public function hasChild ($parent, $child) {
        ...
        return isset($this->_children[$parent->name]) &&
                 in_array($child->name, $this->_children[$parent->name]);
    }
    ...
    public function getAssignment ($roleName, $userId) {
        $this->loadItems();
        $this->loadAssignments($userId);
        if (isset($this->_assignments[$userId][$roleName], 
                $this->_items[$roleName])) {return $this->_items[$roleName];}
    ...
    public function getRolesByUser ($userId) {
        ...
        foreach ($this->getAssignments($userId) as $name => $asgn) {
            if (isset($this->_items[$name]) &&
                     $this->_items[$name]->type == Item::TYPE_ROLE) {
    ...
    public function getPermissionsByRole ($roleName) {
        ...
        foreach (array_keys($result) as $itemName) {
            if (isset($this->_items[$itemName]) &&
                     $this->_items[$itemName] instanceof Permission) {
    ...
    private function getFromCache ($part) {
        if ($this->enableCaching) {return $this->cache->get(
                    $this->buildKey($part));}
    ...
    private function saveToCache ($part, $data) {
        if ($this->enableCaching) {
            $this->cache->set($this->buildKey($part), $data, 
                    $this->cacheDuration, 
                    new TagDependency(
                            [
                                    'tags' => $part
    ...
    private function loadChildren () {
        ...
        if ($this->_children === null &&
                 ($this->_children = $this->getFromCache($part)) === false) {
            $query = (new Query())->from($this->itemChildTable);
            
            $this->_children = [];
            foreach ($query->all($this->db) as $row) {
                if (isset($this->_items[$row['parent']], 
                        $this->_items[$row['child']])) {
    ...
    private function loadRules () {
        ...
        if ($this->_rules === null &&
                 ($this->_rules = $this->getFromCache($part)) === false) {
