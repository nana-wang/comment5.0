...
$this->title = Yii::t('backend','Comment Manage');
				...
				<div class="col-md-12">
					<div class="box box-default collapsed-box box-default-viev">
						...
						<div class="box-header with-border">
							<h3 class="box-title"><?=Yii::t('backend','Review Search');?></h3>
				...
				<div class="col-md-12">
					<div class="box">
						<div class="box-header with-border">
							...
							<p></p><div class="form-group col-md-3">
        ...
        'columns' => [
			...
			'checkboxOptions' => function($model) {
				if( $model->comment_status == 7 || $model->comment_status == 8 ){
					return ['data' => $model->comment_url,'disabled'=>'disabled'];
				}else{
					return ['data' => $model->comment_url];
				}
			...
			[
			'attribute' => 'comment_parent_id',
			'label'=>Yii::t('backend','Comment Type'),
...
'contentOptions'=>['style'=>'width: 5%; text-align: center; vertical-align: middle;'],
			'content'=>
				function($model){
					...
					if( $model->comment_status == 1){
						return '<span class="label label-success" >'.$operation_stat.'</span>' ;
					}elseif( $model->comment_status == 3){
						return '<span class="label label-danger" >'.$operation_stat.'</span>' ;
					}elseif( $model->comment_status == 7){
						return '<span class="label bg-purple" >'.$operation_stat.'</span>' ;
					}else{
...
'contentOptions'=>['style'=>'width: 6%; text-align: center; vertical-align: middle;'],
			...
			'buttons' => [
				'view' => function ($url, $model) {
					...
					return '<a data-pjax="0" aria-label="'.$view.'" title="'.$view.'" href="/admin/index.php?r=comment%2Fview&amp;id='.$model->id.'&from=index"   class="btn btn-default btn-xs"><span class="glyphicon glyphicon-eye-open"></span></a>';
				},
...
function batch_operation(){
    ...
    var suredel = surebulk+text+thisitem;  //您确定要批量【"+text+"】此项吗？
    if(confirm(suredel)){
        $.ajax({
            type: "POST",
